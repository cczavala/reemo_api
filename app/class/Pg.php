...
Class Pg extends Dbconnection
{
	...
	/**
     * Obtiene los datos de un predio PG *
     *
     * @access public
     * @param string $cvePredio
     * @return array $rsrc
     */
    public function getPgInfo($cvePredio)
    {  
		try {
        	$sql = "SELECT CONCAT(p.nombre,' ',p.paterno,' ',p.materno) AS propietario,pd.estatus,
					...
					FROM pg_gral p,estados e,pg_datos pd 
			...
			if ($query) {
				...
				if (!empty( $row )) {
					...
					$rsrc = [
		    			...
		    			"propietario"  => '',
		    			"estatus"      => 2,
		    			"calificacion" => "Clave predio PG no existe"
		    		];
				}
			} else {
